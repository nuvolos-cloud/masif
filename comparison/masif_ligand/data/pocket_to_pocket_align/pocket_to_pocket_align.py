...
def align_pocket_pair(
    pdbfns, cofids, resids, chainids, out_fn_base, cofactor_distance_cutoff=3.0
):
    """
    ...
    for ix, pdbfn in enumerate(pdbfns):
        ...
        for residue in residue_list:
            ...
            if (
                residue.get_resname() == cofids[ix]
                and chain.get_id() == chainids[ix]
                and str(residue.get_id()[1]) == resids[ix]
            ):
                for atom in residue:
    ...
    args = [
        "./TMalign",
        "{}_{}.pdb".format(out_fn_base, 0),
        "{}_{}.pdb".format(out_fn_base, 1),
    ]
...
for training_full_id in training_list_lines:
    ...
    train_pdbfn = os.path.join(
        training_pdb_dir, train_pdbid + "_" + train_chainid + ".pdb"
    )
    ...
    if not os.path.exists(tmpdir):
        os.makedirs(tmpdir)
    tmpfn_base = os.path.join(
        tmpdir, testing_full_id.rstrip() + "_" + training_full_id.rstrip()
    )
    # Perform an alignment using tm alignment.
    tm_score = align_pocket_pair(
        [test_pdbfn, train_pdbfn],
    ...
    print(
        "{},{},{:3f}".format(
            training_full_id.rstrip(), testing_full_id.rstrip(), tm_score
        )
    )
    outtm.write(
        "{},{},{:3f}\n".format(
