...
def get_masif_ligand_prediction(pdbid, probis_test_id, cofid):
    masif_ligand_dir = "masif_pred/test_set_predictions/"
    ...
    ## Ignore heme
    score_for_false = [
        x for ix, x in enumerate(pred_array) if ix != true_val and x != 3
    ]
...
testset_list = open(
    "data/pocket_to_pocket_align/testset_pocket_split_tmscore0.50.txt"
).readlines()
...
masif_list = open(
    "masif_pred/test_set_predictions/all_masif_kripo_names.txt"
).readlines()
    ...
    if cofid_test == cofid_train:
        ...
        print(
            "{} {} {} {} {}".format(
                ...
                pdbid_train,
                cofid_train,
                myjson[0]["alignment"][0]["scores"]["z_score"],
            )
        )
...
for key in correct_by_class_probis:
    if total_by_class[key] > 0:
        ...
        num_classes += 1.0
        print(
            "{}: {:.2f}".format(key, correct_by_class_probis[key] / total_by_class[key])
        )
...
for key in correct_by_class_probis:
    if total_by_class[key] > 0:
        ...
        num_classes += 1.0
        print(
            "{}: {:.2f}".format(key, correct_by_class_masif[key] / total_by_class[key])
        )
print("Num classes: {}".format(num_classes))
print("Balanced accuracy = {:.2f}".format(100 * balanced_acc / num_classes))
...
labels = np.concatenate(
    [np.ones_like(masif_true_scores), np.zeros_like(masif_false_scores)], axis=0
)
...
labels = np.concatenate(
    [np.ones_like(probis_true_scores), np.zeros_like(probis_false_scores)], axis=0
)
