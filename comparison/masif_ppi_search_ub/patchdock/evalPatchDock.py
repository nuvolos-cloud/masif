...
gt_ix = []
for ix in range(len(all_scores[:1000])):
...
if not ground_truth_present:
    ...
    if len(gt_ix) > 0:
        transo = Popen(
            [trans_output_bin, pdbid, "1", "{}".format(len(gt_ix))],
            cwd=run_dir,
            stdout=PIPE,
            stdin=PIPE,
        )
        ...
        for ix in range(len(gt_ix)):
            # Open the PDB.
            rmsd = test_alignment(
                target_pdb_fn, source_pdb_fn, aligned_pdb_fn.format(ix + 1)
            )
            if rmsd < 5.0:
                found = 1
                outfile.write(
                    "{} {} {} {}\n".format(
                        gt_ix[ix], rmsd, all_scores[gt_ix[ix]], all_names[gt_ix[ix]]
                    )
                )
                break
    if found == 0:
